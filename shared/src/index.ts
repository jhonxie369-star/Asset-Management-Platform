...
export interface LiveEndpoint {
  ...
  port: number;
  alive: boolean;
  banner?: string;        // TCP 连接时抓到的原始 banner
  firstSeenAt: string;
  lastSeenAt: string;
...
export type ServiceProtocol =
  ...
  | 'mssql' | 'oracle' | 'memcached' | 'zookeeper' | 'neo4j' | 'etcd' | 'rabbitmq' | 'kafka'
  | 'tidb' | 'oceanbase' | 'opensearch' | 'solr' | 'rocketmq' | 'pulsar'
  | 'doris' | 'trino' | 'presto' | 'hive' | 'hdfs' | 'consul' | 'nacos';
...
export interface Service {
  id: string;
  endpointId: string;     // 关联到 LiveEndpoint
  assetId: string;
  /** 兼容旧字段: 可为 IP 或 hostname */
  ip: string;
  host?: string;
...
export interface TaskTargetSelector {
  ...
  assetIds?: string[];
  assetListId?: string;
  serviceFilter?: { protocol?: ServiceProtocol[]; portRange?: [number, number] };
  assetFilter?: { assetKinds?: AssetKind[]; q?: string; tags?: string[] };
}
...
export interface Task {
  ...
  schedule?: { cron?: string; intervalMinutes?: number; everyDays?: number };
  status: TaskStatus;
  createdAt: string;
  updatedAt: string;
  lastRunAt?: string;
...
export interface Run {
  ...
  counters: { total: number; success: number; failed: number };
  startedAt: string;
  finishedAt?: string;
  error?: string;
}
...
// ─── Finding / Knowledge Layer ──────────────────────────────────

export type FindingType =
  | 'weak_password' | 'unauth' | 'anonymous_login' | 'default_credential'
  | 'plaintext_protocol' | 'auth_exposure'
  | 'exposure' | 'dangerous_fingerprint' | 'sensitive_path'
  | 'new_endpoint' | 'new_service' | 'endpoint_gone'
  | 'fingerprint_change' | 'config_drift';
...
export interface RiskSnapshot {
  id: string;
  takenAt: string;            // ISO 时间
  date: string;               // 'YYYY-MM-DD',方便按日去重
  instanceKey: string;
  cloud?: string;
  instanceName?: string;
  score: number;
  bySeverity: Record<string, number>;  // {critical, high, medium, low, info}
  findingCount: number;
}

// ─── API Key / Automation Layer ────────────────────────────────
...
export function entryIp(e: string | AssetListEntry): string {
  return typeof e === 'string' ? e : e.ip;
}

/** 把 entry 升格为 AssetListEntry(字符串则落 source=manual) */
export function entryToObject(e: string | AssetListEntry): AssetListEntry {
...
export interface DashboardStats {
  ...
  recentRuns: Run[];
  topRisks: Finding[];
  assetsByZone: { public: number; private: number };
}
